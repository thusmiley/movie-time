...
const page = ({ params }) => {
  ...
  const reRenderData = () => {
    ...
    fetch(`https://api.themoviedb.org/3/tv/${params.id}/images`, options)
      .then((response) => response.json())
      .then((response) => {
        setPhotos(response);
      })
    ...
    fetch(
      `https://api.themoviedb.org/3/tv/${params.id}/similar?language=en-US&page=1`,
      options
    )
      .then((response) => response.json())
      .then((response) => {
          ...
          <div className="md:flex md:justify-between md:items-start md:space-x-10">
            ...
            <div className="mt-6 md:w-1/2 md:mt-0">
              ...
              <p className="text-[14px] font-light text-white/75 flex items-center">
                TV Series<span className="px-2">•</span>
                {tv?.first_air_date.toString().slice(0, 4)} -{" "}
                {tv?.last_air_date.toString().slice(0, 4)}
              ...
              <p className="mt-8">{tv?.overview}</p>
            </div>
          </div>
